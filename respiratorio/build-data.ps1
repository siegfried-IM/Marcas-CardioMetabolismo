param(
  ...
  # lookup legacy en $dashboardDir con el patron 'AR_PM_FV_Standard*'.
...
$pmPath = Get-MatchingPath -Dir $(if ($IqviaDir) { $IqviaDir } else { $dashboardDir }) -Include $(if ($IqviaPattern) { $IqviaPattern } else { 'AR_PM_FV_Standard*' })
